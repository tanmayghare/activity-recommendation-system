...
def train_fer_model():
    ...
    try:
        ...
        if not config.fer_config.train_data_dir.exists():
            logger.warning(f"Training dataset directory not found: {config.fer_config.train_data_dir}")
            logger.info("Please download the dataset and place it in the correct location.")
            logger.info("Expected structure: {config.fer_config.train_data_dir}/<emotion>/<image_files>")
            return False
            
        if not config.fer_config.val_data_dir.exists():
            logger.warning(f"Validation dataset directory not found: {config.fer_config.val_data_dir}")
            logger.info("Please download the dataset and place it in the correct location.")
            logger.info("Expected structure: {config.fer_config.val_data_dir}/<emotion>/<image_files>")
            return False
        
        # Initialize and train the model
        fer_model = FERModel(
            base_path=str(config.fer_dataset_dir),
...
def main():
    ...
    parser.add_argument("--fer", action="store_true", help="Train facial emotion recognition model")
